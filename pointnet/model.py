...
class STN4d(nn.Module):
    ...
    def forward(self, x):
        ...
        if x.is_cuda:
            iden = iden.to(device = self.device)
        x = x.view(-1, 4, 3)
        x = x + iden
...
class STNkd(nn.Module):
    def __init__(self, k=64, device='cuda:1'):
...
class PointNetfeat(nn.Module):
    def __init__(self, device, channel = 4, global_feat = True, feature_transform = False):
        super(PointNetfeat, self).__init__()
        self.c = channel
        self.stn4 = STN4d(device)
        self.stn3 = STN3d(device)
        self.conv1 = torch.nn.Conv1d(3, 64, 1)
        self.conv2 = torch.nn.Conv1d(64, 128, 1)
        self.conv3 = torch.nn.Conv1d(128, 1024, 1)
        self.bn1 = nn.BatchNorm1d(64)
...
class PointNetCls(nn.Module):
    def __init__(self, k=2, device = 'cuda:1', feature_transform=False):
        ...
        self.feature_transform = feature_transform
        self.feat = PointNetfeat(device, channel = 3, global_feat=True, feature_transform=feature_transform)
        self.fc1 = nn.Linear(1024, 512)
        self.fc2 = nn.Linear(512, 256)
...
if __name__ == '__main__':
    ...
    trans = STNkd(k=64, deivce = 'cuda:1')
